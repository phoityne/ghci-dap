...
import GHC ( LoadHowMuch(..), Target(..),  TargetId(..),
             ...
             getModuleGraph, handleSourceError, ms_mod, BreakpointId (..), InternalBreakpointId (..) )
...
toBreakIdAndLocation (Just inf) = do
  ...
  return $ listToMaybe [ id_loc | id_loc@(_,loc) <- IntMap.assocs (breaks st),
                                  breakModule loc == GHC.ibi_tick_mod inf,
                                  breakTick loc == GHC.ibi_tick_index inf]
...
ignoreCmd argLine = withSandboxOnly ":ignore" $ do
    ...
    case result of
      Left sdoc -> printForUser sdoc
      Right (loc, count)   -> do
        let breakInfo = GHC.BreakpointId (breakModule loc) (breakTick loc)
